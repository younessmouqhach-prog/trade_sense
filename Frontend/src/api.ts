  ...
  const getApiBaseUrl = () => {
    ...
    if (typeof (import.meta as any).env !== 'undefined' && (import.meta as any).env.VITE_API_URL) {
      return (import.meta as any).env.VITE_API_URL;
    }
    
    // If running on localhost, use local backend by default
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      return 'http://localhost:5000/api';
    }
    
    // Default to production URL
    return 'https://trade-sense-hi0t.onrender.com/api';
  };
  ...
  export const apiRequest = async (endpoint: string, options: RequestInit = {}) => {
    ...
    const response = await fetch(url, {
      ...config,
      signal: AbortSignal.timeout(60000) // 60 second timeout to allow Render/Cold start time
    });
